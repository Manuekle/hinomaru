<script lang="ts">
	...
	let isDark = $derived($theme === 'dark' || ($theme === 'system' && typeof window !== 'undefined' && window.matchMedia('(prefers-color-scheme: dark)').matches));
...
<div class="landing-root">
				...
				<div class="hero-phone-container">
					<div class="iphone-16-mockup">
						...
						<div class="mockup-screen">
							<img src="/landing_hero.png" alt="Hinomaru App" class="app-screenshot" />
						</div>
						<!-- Device Frame -->
				...
				{#each previewDecks as deck}
					<a href="/deck/{deck.id}" class="deck-preview-card">
						<div class="deck-preview-left">
							...
							<span class="deck-preview-title">{$locale === 'es' ? deck.title_es : deck.title_en}</span>
						</div>
	...
	<section class="features-section">
		<div class="section-header text-center">
			...
			<h2 use:inView>{t('landing.features.label', $locale)}</h2>
		</div>
				...
				<div class="feature-item" style="background: {feature.color}">
					...
					<div class="feature-image">
						<div class="iphone-16-mockup mini">
							<div class="mockup-screen">
								<img src={getFeatureImg(feature.id, $locale, isDark)} alt={feature.title} class="app-screenshot" />
							</div>
							<img src="/mockups/iphone16pro_frame.png" alt="iPhone 16 Pro" class="device-frame" />
						</div>
					</div>
	...
	<section class="final-cta">
		<div class="final-card" use:inView={{ y: 36, duration: 0.7 }}>
			<div class="final-content">
				<h2>{t('landing.cta.title', $locale)}</h2>
	...
	<footer class="landing-footer">
		<div class="footer-grid">
			...
			<div class="footer-links-col">
				...
				<a href="https://ko-fi.com/manujsx" target="_blank" rel="noopener noreferrer" class="kofi-link-badge">
					<Icon icon={Coffee02Icon} size={16} />
					{t('footer.support', $locale)}
				</a>
			</div>
		</div>
		<div class="footer-bottom">
			<p>{t('landing.copyright', $locale).replace('{year}', String(new Date().getFullYear()))} {t('landing.footer.crafted', $locale)}</p>
		</div>
	</footer>
</div>
...
<style>
	...
	.hero-float-card.left { top: 30%; left: -60px; }
	.hero-float-card.right { bottom: 15%; right: -40px; }
	.float-row { display: flex; align-items: center; gap: 12px; }
	.float-emoji { font-size: 28px; }
	.jp-text { font-family: var(--font-jp); font-weight: 700; font-size: 15px; margin-bottom: 4px; }
	.float-lines .bar { height: 4px; background: var(--ink-100); border-radius: 2px; }
	...
	.level-pill:hover { transform: translateY(-2px); box-shadow: var(--shadow-md); border-color: var(--brand-primary); }
	.level-name { color: var(--brand-primary); font-size: 18px; }
	...
	.deck-preview-card:hover { border-color: var(--brand-primary); transform: translateX(4px); }
	.deck-preview-left { display: flex; align-items: center; gap: 16px; }
	.deck-level-pill { background: var(--brand-primary); color: #fff; padding: 4px 10px; border-radius: 8px; font-weight: 800; font-size: 13px; }
	.deck-preview-title { font-weight: 700; font-size: 17px; }
	.deck-preview-arrow { font-size: 20px; opacity: 0.3; }
	.preview-all-link {
		font-size: 16px;
		font-weight: 800;
		color: var(--brand-primary);
	...
	.stat-label { font-size: 14px; font-weight: 700; color: var(--fg-tertiary); text-transform: uppercase; }
	...
	.feature-item:nth-child(even) .feature-content { direction: ltr; }
	.feature-content { padding: 80px; }
	.feature-content h3 { font-size: 32px; font-weight: 900; color: var(--sumi); margin-bottom: 20px; }
	.feature-content p { font-size: 18px; color: var(--fg-secondary); line-height: 1.6; }
	.feature-image {
		height: 100%;
		display: flex;
		align-items: center;
	...
	.test-quote { font-size: 20px; font-weight: 500; color: var(--sumi); line-height: 1.6; margin-bottom: 32px; font-style: italic; }
	.test-author { display: flex; align-items: center; gap: 16px; }
	.author-avatar { width: 48px; height: 48px; background: var(--ink-50); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 24px; }
	.author-name { font-weight: 800; font-size: 17px; color: var(--sumi); }
	.author-sub { font-size: 14px; color: var(--fg-tertiary); font-weight: 600; }
	...
	.final-content h2 { font-size: 40px; font-weight: 900; margin-bottom: 16px; }
	.final-content p { font-size: 18px; opacity: 0.8; margin-bottom: 40px; }
	...
	.cta-link-btn:hover { opacity: 1; text-decoration: underline; }

	.final-visual { background: var(--brand-primary); position: relative; display: flex; align-items: center; justify-content: center; }
	.sunburst { position: absolute; width: 100%; height: 100%; background: radial-gradient(circle, rgba(255,255,255,0.2) 0%, transparent 70%); }
	.final-emoji { font-size: 120px; position: relative; z-index: 2; }
	...
	.brand-wrap { display: flex; align-items: center; gap: 10px; margin-bottom: 16px; }
	.brand-dot { width: 12px; height: 12px; background: var(--brand-primary); border-radius: 50%; }
	.brand-name { font-size: 22px; font-weight: 900; letter-spacing: -0.02em; }
	.brand-tagline { font-size: 15px; opacity: 0.7; line-height: 1.5; margin-bottom: 24px; color: var(--fg-secondary); }

	.footer-links-col h4 { font-size: 14px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.1em; margin-bottom: 24px; color: var(--brand-primary); }
	.footer-links-col a { display: block; color: var(--fg-primary); opacity: 0.7; text-decoration: none; margin-bottom: 12px; font-size: 15px; transition: all 200ms; }
	.footer-links-col a:hover { opacity: 1; transform: translateX(4px); }

	.kofi-link-badge { opacity: 1 !important; }
	.kofi-link-badge img { height: 44px; transition: transform 200ms; }
	.kofi-link-badge:hover { transform: none !important; }

	.footer-bottom { max-width: 1100px; margin: 0 auto; padding-top: 32px; border-top: 1px solid var(--ink-200); text-align: center; font-size: 14px; opacity: 0.6; color: var(--fg-secondary); }

	/* ── RESPONSIVE ── */
	@media (max-width: 900px) {
		.feature-item, .final-card, .footer-grid { grid-template-columns: 1fr; text-align: center; }
		.feature-item:nth-child(even) { direction: ltr; }
		.feature-content { padding: 40px; }
		.feature-image { order: -1; height: 320px; }
		.stats-grid { grid-template-columns: 1fr; }
		.final-visual { height: 240px; }
		.final-content { padding: 40px; align-items: center; }
		.brand-wrap { justify-content: center; }
		.footer-grid { gap: 40px; }
		.final-actions-row { justify-content: center; }
		.hero-actions { flex-direction: column; align-items: stretch; max-width: 320px; margin: 0 auto 40px; }
	}
	@media (max-width: 600px) {
		.hero-float-card { display: none; }
		.hero-section h1 { font-size: 40px; }
		.level-pill { width: 100%; justify-content: center; }
	}
</style>
