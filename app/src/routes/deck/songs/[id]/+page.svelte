...
{#if !song}
	<div
		style="padding:80px 24px;text-align:center;color:var(--fg-secondary);max-width:680px;margin:0 auto;"
	>
				...
				<div class="ctrl-row">
					...
					<div class="speed-wrap">
						<button class="speed-btn" class:active={speed === 0.75} onclick={() => setSpeed(0.75)}
							>0.75×</button
						>
						<button class="speed-btn" class:active={speed === 1} onclick={() => setSpeed(1)}
							>1×</button
						>
					</div>
				</div>
		...
		{#if hasLyrics}
			<div class="section" use:fadeUp={{ delay: 0.2, y: 10 }}>
				...
				<div class="lyrics-scroll" bind:this={lyricsContainer}>
					{#each song.lyrics as line, idx (idx)}
