...
<div class="session-layout">
	<div class="premium-header" use:fadeIn={{ delay: 0 }}>
		<button class="premium-header-back" onclick={() => (showExitModal = true)}>
			← {t('deck.back', $locale)}
		</button>

		<div class="premium-header-main">
			<div class="premium-header-left">
				<span class="premium-label-pill">{phase === 'exam' && currentQuestion ? typeLabel(currentQuestion.type) : t('mode.exam.title', $locale)}</span>
			</div>
			
			<div class="premium-step-indicator">
				<span class="premium-step-curr">{currentIdx + 1}</span>
				<span class="premium-step-divider">/</span>
				<span class="premium-step-total">{questions.length}</span>
			</div>

			<div class="premium-header-right">
				<div class="premium-timer-pill" class:is-critical={timerCritical}>
	...
	<div class="session-container">
		{#if phase === 'exam' && currentQuestion}
			<div class="question-wrap" use:fadeUp={{ delay: 0, y: 12 }}>
			...
			<StickyFooter>
				{#if !checked}
					<button
						class="hm-btn hm-btn-dark hm-btn-full"
				...
				{:else}
					<button class="hm-btn hm-btn-dark hm-btn-full" onclick={advanceQuestion}>
		...
		{:else if phase === 'result'}
			...
			<div use:fadeUp={{ delay: 0, y: 20 }} class="result-screen">
				...
				<div class="result-actions" use:fadeUp={{ delay: 0.5, y: 12 }}>
					<button class="hm-btn hm-btn-secondary" style="flex:1;" onclick={() => goto(`/deck/${deck?.id}`)}>
						← {t('deck.back', $locale)}
					</button>
					<button class="hm-btn hm-btn-dark" style="flex:1;" onclick={startExam}>
						{t('exam.retry', $locale)}
					</button>
				</div>
			</div>
...
<style>
	...
	.result-premium-hero.is-pass {
		background: linear-gradient(135deg, var(--hinomaru-red) 0%, #8b0021 100%);
	}
	...
	.hero-main-title { font-size: 32px; font-weight: 800; margin: 0; letter-spacing: -0.02em; }
	...
	.stat-v { font-size: 18px; font-weight: 800; color: var(--sumi); }
	.stat-l { font-size: 12px; font-weight: 700; color: var(--fg-tertiary); }
