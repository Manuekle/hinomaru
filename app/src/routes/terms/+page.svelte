<script lang="ts">
	...
	const sections = [
		{ id: 's1', icon: DocumentValidationIcon },
		{ id: 's2', icon: FileEditIcon },
		{ id: 's3', icon: Notebook01Icon },
		{ id: 's4', icon: InformationCircleIcon },
		{ id: 's5', icon: LicenseDraftIcon }
	];
</script>
						...
						{#each sections as s, i (s.id)}
							<li>
								<a href="#{s.id}">
									...
									<span>{t(`terms.${s.id}.title`, $locale)}</span>
								</a>
							</li>
						{/each}
					</ul>
					...
					<section id={s.id} use:inView={{ y: 24, duration: 0.55 }}>
						<div class="section-head">
							...
							<div class="section-meta">
								...
								<h2>{t(`terms.${s.id}.title`, $locale)}</h2>
							</div>
						</div>
						<p>{t(`terms.${s.id}.desc`, $locale)}</p>
					</section>
				{/each}
			</div>
		</div>
